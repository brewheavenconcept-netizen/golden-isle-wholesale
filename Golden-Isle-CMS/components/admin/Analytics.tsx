...
export default function Analytics() {
    ...
    useEffect(() => {
        if (!storeId) {
            // storeId not yet available — stop spinner so empty state shows
            setLoading(false);
            return;
        }
        setLoading(true);
    ...
    if (!loading && orders.length === 0) return (
        <div className="space-y-6">
            ...
            <div className="bg-white dark:bg-[#111111] rounded-xl border border-slate-200 dark:border-white/10 p-16 flex flex-col items-center justify-center text-center shadow-sm">
                <div className="w-16 h-16 bg-blue-50 dark:bg-blue-900/20 rounded-2xl flex items-center justify-center mb-4">
                    <TrendingUp className="w-8 h-8 text-blue-500" />
                </div>
                <h3 className="text-xl font-bold text-slate-800 dark:text-white mb-2">No Data Yet</h3>
