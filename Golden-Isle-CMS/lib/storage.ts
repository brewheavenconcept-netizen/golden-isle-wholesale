...
export async function updateProduct(
    ...
    if (u.images !== undefined) {
        payload.image_url = u.images.length > 0 ? u.images[0] : null;
    } else if (u.image_url !== undefined) {
        payload.image_url = u.image_url;
    }
...
export async function getOrder(id: string, storeId?: string): Promise<Order | null> {
    const query = supabase.from('orders').select('*').eq('id', id);
...
export async function seedProducts(storeId: string): Promise<void> {
    ...
    const sampleProducts = [
        { name: 'Tiger Beer 24x330ml', price: 78.00, category: 'Craft Beer', stock_status: 'in_stock', store_id: storeId, description: 'Classic Tiger lager.', image_url: null, is_featured: false, created_at: new Date().toISOString() },
        { name: 'Heineken 24x330ml', price: 88.00, category: 'Craft Beer', stock_status: 'in_stock', store_id: storeId, description: 'Premium Dutch pilsner.', image_url: null, is_featured: false, created_at: new Date().toISOString() },
    ];
